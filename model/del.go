...
//读写分离
func DelRedis(config tool.Config) {
	RedisReadDB = redis.NewClient(&redis.Options{
		Addr:     tool.StrSplicing(config.Redis.SlaveIP, ":", config.Redis.SlavePort),
		Password: config.Redis.SlaveAuth, //若主不设置密码，redis主从起不来 具体看redis配置
		DB:       config.Redis.SlaveDB,
	})
	RedisWriteDB = redis.NewClient(&redis.Options{
		Addr:     tool.StrSplicing(config.Redis.MasterIP, ":", config.Redis.MasterPort), //这里不能两个都用回环，走的是网络层，需要模拟两个网段
		Password: config.Redis.MasterAuth,
		DB:       config.Redis.MasterDB,
	})
	//系统redis客户端
	RedisSysDB = redis.NewClient(&redis.Options{
		Addr:     tool.StrSplicing(config.Redis.MasterIP, ":", config.Redis.MasterPort),
		Password: config.Redis.MasterAuth,
		DB:       config.Redis.MasterDB,
	})
	...
	log.Println(pong, err)
	if err := RedisReadDB.SlaveOf(config.Redis.SlaveOfIp, config.Redis.MasterPort).Err(); err != nil { //redis主从
...
func DelMysql(config tool.Config) {
	db, err := gorm.Open(config.Mysql.Name, config.Mysql.Dns)
	if err != nil {
		panic(err)
	}
	//GORM日志文件输出待做
	...
	db.DB().SetMaxIdleConns(config.Mysql.MaxIdleConns) //最大可空闲连接数
	...
	db.DB().SetMaxOpenConns(config.Mysql.MaxOpenConns) //最大连接数
	...
	db.DB().SetConnMaxLifetime(time.Second * time.Duration(config.Mysql.ConnMaxLifetime)) //一个连接使用的最大时长
	...
	db.AutoMigrate(&User{}, &Article{}, &Stat{}, &Follower{}, &Comment{})
	DB = db
